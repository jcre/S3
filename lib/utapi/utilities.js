...
export function pushMetric(action, log, metricObj) {
    ...
    if (authInfo) {
        ...
        const arn = authInfo.getArn();
        const shortId = authInfo.getShortid();
        if (arn && shortId) {
            const arnSchemaArr = arn.split(':');
            const arnId = arnSchemaArr[arnSchemaArr.length - 2];
            // If the ID in the ARN is different from the shortId, then it's the
            // user's ID and so we push metrics for the user-level.
            utapiObj.userId = arnId !== shortId ? shortId : undefined;
        }
    }
