...
class Config {
    ...
    _getConfig() {
        ...
        if (config.regions !== undefined) {
            assert(config.restEndpoints === undefined,
        ...
        if (!config.regions && !config.restEndpoints) {
            throw new Error('bad config: config must include one of either ' +
                'regions or restEndpoints');
        }
        ...
        if (process.env.S3DATA) {
            ...
            if (process.env.S3DATA === 'multiple') {
                assert(this.locationConstraints !== undefined &&
                    ...
                    'and restEndpoints must be set'
                );
            }
            if (process.env.S3DATA === 'scality') {
                assert(this.regions !== undefined &&
                    this.locationConstraints === undefined &&
                    this.restEndpoints === undefined,
                    'if data is set to scality, legacy version of ' +
                    'config must be used'
                );
            }
