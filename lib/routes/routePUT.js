...
export default function routePUT(request, response, log, statsClient) {
    ...
    if (request.objectKey === undefined) {
        ...
        const post = [];
        let postLength = 0;
        // Send continue message for streaming V4 requests, if applicable.
                ...
                if (request.post) {
                    ...
                    return parseString(xmlToParse, (err, result) => {
                        ...
                        log.trace('location constraint',
                            { locationConstraint });
                        return api.callApiMethod('bucketPut', request, response,
                            log, (err, corsHeaders) => {
                                statsReport500(err, statsClient);
                                const resHeaders = corsHeaders ?
                                    Object.assign({}, location, corsHeaders) :
                                    location;
                                return routesUtils.responseNoBody(err,
                                    resHeaders, response, 200, log);
                            }, locationConstraint);
                    });
                }
