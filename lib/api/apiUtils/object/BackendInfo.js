...
export class BackendInfo {
    ...
    */
    ...
    static isValidLocationConstraint(locationConstraint, log) {
        if (config.regions &&
            Object.keys(config.regions).indexOf(locationConstraint) < 0) {
            log.trace('proposed locationConstraint is invalid',
                { locationConstraint });
            return false;
        } else if (!config.regions && config.locationConstraints &&
    ...
    static isValidRequestEndpoint(requestEndpoint, log) {
        if (utils.getAllEndpoints().indexOf(requestEndpoint) < 0) {
            log.trace('requestEndpoint does not match config restEndpoints',
                { requestEndpoint });
            return false;
        }
        if (!config.regions && config.locationConstraints &&
            Object.keys(config.locationConstraints).indexOf(config
